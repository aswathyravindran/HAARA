...
<table border="1" align="center">
    ...
    <tr>
        ...
        <td>
            ...
            if (p == 1) {
            %>
            <button onclick="cancel(<%=rs.getString("request_id")%>, 'furniture')">Cancel</button>
            <% } 
            } else {
            %>Reject<% }%>
        </td>
    ...
    <tr>
        ...
        <td>
            ...
            <%if (p == 1) {
                if (differenceDays > 0) {
            %>  
            <button onclick="cancel(<%=rs1.getString("request_id")%>, 'property')">Cancel</button>
            <% }
            %>Paid
            <% } else if (p == 0) {
            %>
...
<%
    if (request.getParameter("can") != null) {
        ...
        if (type.equals("property")) {
            upQry = "update tbl_request set request_status=3 where request_id='" + id + "'";
...
<script>
function cancel(rid, type) {
    var result = confirm("Are you sure you want to cancel? You will lose 10% of your paid amount on cancellation!");
    if (result) {
        window.location.href = "UViewRequest.jsp?can=" + rid + "&type=" + type;
    } else {
